resource "aws_route_table" "access" {
  count  = local.egress || local.ingress ? 1 : 0
...
resource "aws_route_table_association" "access" {
  for_each       = tomap(local.egress || local.ingress ? local.access_subnets : {})
...
resource "aws_route_table" "app" {
  for_each = tomap(local.egress ? local.access_subnets : { "app" : {} })
  ...
  dynamic "route" {
    ...
    # at the nat gw
    for_each = local.egress ? [each.value.subnet_name] : []
    content {
      cidr_block     = "0.0.0.0/0"
...
resource "aws_route_table_association" "app" {
  for_each       = tomap(local.egress ? local.app_route_table_association_egress : local.app_route_table_association_noegress)
